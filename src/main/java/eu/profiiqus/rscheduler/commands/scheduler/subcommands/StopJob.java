...
/**
 * Stop Job is a wrapper for scheduler's stop subcommand
 *
 * @author Prof
 */
public class StopJob implements Subcommand {

    private FileConfiguration config;
    private JobManager jobManager;

    /**
     * Stop Job constructor
     */
    public StopJob() {
    ...
    /**
     * Execute executes the job's stop functionality - halts the jobs run
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(sender instanceof Player) {
            ...
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', this.config.getString("messages.job-stop-success").replace("{JOB}", job.getId())));
            job.setRunning(false);
        } else {
            if(args.length == 1) {
                Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', this.config.getString("messages.job-stop-usage")));
                return;
            }
            ...
            Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', this.config.getString("messages.job-stop-success").replace("{JOB}", job.getId())));
            job.setRunning(false);
        }
    }
}
